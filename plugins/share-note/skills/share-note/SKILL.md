...
- For setup, prefer the one-command `setup-browser` workflow in `references/setup.md`; it verifies the token and binds the project automatically. Use one restricted JSON request file and call `node <absolute-client-path> <action> --request <absolute-request-path>`. Do not put note bodies, keys, complete fragment URLs, or user text in shell arguments.
